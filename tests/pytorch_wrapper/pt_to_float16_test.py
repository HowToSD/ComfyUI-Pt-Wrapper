...
from pytorch_wrapper.pt_to_float16 import PtToFloat16


class TestPtToFloat16(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.node = PtToFloat16()

        # Test cases: (Tensor A, expected result, expected dtype)
        self.test_cases = [
            (torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32), torch.tensor([1.0, 2.0, 3.0], dtype=torch.float16), torch.float16),
        ]
