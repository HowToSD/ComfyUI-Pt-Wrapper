...
from pytorch_wrapper.pt_compute_loss import PtComputeLoss 
...
class TestComputeLoss(unittest.TestCase):
    def setUp(self) -> None:
        ...
        """
        self.compute_node = PtComputeLoss()
        self.loss_node = PtnL1Loss()
        self.loss_fun_none = self.loss_node.f("none")[0]
        self.loss_fun_mean = self.loss_node.f("mean")[0]
        self.loss_fun_sum = self.loss_node.f("sum")[0]
    ...
    def test_none(self) -> None:
        ...
        for y_hat, y in self.test_cases:
            with self.subTest(y_hat=y_hat):
                actual = self.compute_node.f(y_hat, y, self.loss_fun_none)[0]
    ...
    def test_mean(self) -> None:
        ...
        for y_hat, y in self.test_cases:
            with self.subTest(y_hat=y_hat):
                ...
                actual = self.compute_node.f(y_hat, y, self.loss_fun_mean)[0]
    ...
    def test_sum(self) -> None:
        ...
        for y_hat, y in self.test_cases:
            with self.subTest(y_hat=y_hat):
                ...
                actual = self.compute_node.f(y_hat, y, self.loss_fun_sum)[0]
                self.assertTrue(torch.equal(actual, expected),
                                f"Expected {expected}, got {actual}")
