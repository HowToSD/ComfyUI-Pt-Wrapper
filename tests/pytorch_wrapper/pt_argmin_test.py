...
from pytorch_wrapper.pt_argmin import PtArgmin


class TestPtArgmin(unittest.TestCase):
    def setUp(self):
        ...
        self.test_cases = [
            (torch.tensor([[1, 3, 2], [4, 0, 5]]), "0", False, torch.tensor([0, 1, 0])),
            (torch.tensor([[1, 3, 2], [4, 0, 5]]), "1", True, torch.tensor([[0], [1]])),
            (torch.tensor([[1, 3, 2], [4, 0, 5]]), "", False, torch.tensor(4)),
        ]

    def test_argmin(self):
        """Test torch.argmin behavior."""
