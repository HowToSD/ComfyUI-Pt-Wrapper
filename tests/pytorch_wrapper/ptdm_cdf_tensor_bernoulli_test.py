...
class TestPtdfCdfBernoulli(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.test_cases = {
            "Bernoulli": [
                (0.4, 0, scst.bernoulli.cdf(0, 0.4)),
                (0.4, 1, scst.bernoulli.cdf(1, 0.4)),
                (0.4, [0, 1], scst.bernoulli.cdf([0, 1], 0.4))  # 1d array for x
            ]
        }
    ...
    def test_custom_class_1(self):
        ...
        expected = torch.tensor(0.6).numpy()
    ...
    def test_custom_class_3(self):
        d = BernoulliEx(probs=0.4)
        actual = d.cdf(torch.tensor([0.0, 1.0])).numpy()
        expected = torch.tensor([0.6, 1.0]).numpy()
