...
from pytorch_wrapper.ptdm_pdf import PtdmPdf
...
class TestPtdfPdf(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.test_cases = {
            "Beta": [(2.0, 5.0, 0.3, scst.beta.pdf(0.3, 2, 5))],
            "Gamma": [(2.0, 1.0, 3.0, scst.gamma.pdf(3.0, 2, scale=1))],
            "Normal": [(0.0, 1.0, -1.5, scst.norm.pdf(-1.5, 0, 1))],
            "Exponential": [(1.0, 3.0, scst.expon.pdf(3.0, scale=1))],
            "Dirichlet": [(torch.tensor([1.0, 1.0, 1.0]), torch.tensor([0.2, 0.5, 0.3]), scst.dirichlet.pdf([0.2, 0.5, 0.3], [1.0, 1.0, 1.0]))],
            "VonMises": [(0.0, 1.0, 1.0, scst.vonmises.pdf(1.0, kappa=1.0, loc=0))],
            "StudentT": [(1.0, 0.0, scst.t.pdf(0.0, 1))],
            "Uniform": [(0.0, 1.0, 0.5, scst.uniform.pdf(0.5, loc=0, scale=1))],
        }
            ...
            for case in cases:  # For each distribution's test cases
                with self.subTest(distribution=dist_name, params=case):
                    if dist_name == "Binomial":
                        ...
                        pdf = PtdmPdf().f(distribution, str(case[1]))[0]  # k
                    else:
                        ...
                        if dist_name == "Dirichlet":
                            pdf = PtdmPdf().f(distribution, str(k_tensor.tolist()))[0]
                        else:
                            pdf = PtdmPdf().f(distribution, str(k_tensor.item()))[0]
