...
from pytorch_wrapper.pt_to_image_transpose import PtToImageTranspose


class TestPtToImageTranspose(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.node = PtToImageTranspose()

        # Define test cases: (input_tensor, expected_shape)
        self.test_cases = [
            # Case 1: 3D tensor (C, H, W) -> (H, W, C)
            (torch.ones(3, 64, 128), (64, 128, 3)),  # (3,64,128) -> (64,128,3)

            # Case 2: 4D tensor (N, C, H, W) -> (N, H, W, C)
            (torch.ones(10, 3, 64, 128), (10, 64, 128, 3)),  # (10,3,64,128) -> (10,64,128,3)
            ...
            (torch.ones(10, 3, 64, 128, 2), ValueError),  # 5D tensors should raise an error
        ]

    def test_transpose(self):
        """Test tensor to image transposition with valid and invalid cases."""
