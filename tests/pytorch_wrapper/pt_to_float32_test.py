...
from pytorch_wrapper.pt_to_float32 import PtToFloat32


class TestPtToFloat32(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.node = PtToFloat32()

        # Test cases: (Tensor A, expected result, expected dtype)
        self.test_cases = [
            (torch.tensor([1.0, 2.0, 3.0], dtype=torch.float16), torch.tensor([1.0, 2.0, 3.0], dtype=torch.float32), torch.float32),
        ]
