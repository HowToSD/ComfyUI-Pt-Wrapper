...
from pytorch_wrapper.pt_int_create import PtIntCreate

class TestPtIntCreate(unittest.TestCase):
    def setUp(self):
        """Set up test data."""
        self.node = PtIntCreate()

        # Test cases covering rank 0 (scalar) to rank 5 tensors
        self.test_cases = [
            # Rank 0 (scalar)
            ("12345", torch.tensor(12345, dtype=torch.int32)),

            # Rank 1 (1D array)
            ("[1, 2, 3]", torch.tensor([1, 2, 3], dtype=torch.int32)),

            # Rank 2 (2D matrix)
            ("[[1, 2, 3], [4, 5, 6]]", torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=torch.int32)),
    ...
    def test_create_tensor(self):
        ...
        for data_str, expected_tensor in self.test_cases:
            with self.subTest(data=data_str):
                ...
                self.assertEqual(retval.dtype, torch.int32, f"Dtype mismatch for input: {data_str}")
