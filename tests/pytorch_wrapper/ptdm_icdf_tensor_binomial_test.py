...
class TestPtdfIcdfBinomial(unittest.TestCase):
    def setUp(self):
        """Set up test instance."""
        self.test_cases = {
            "Binomial": [
                (10.0, 0.5, 0.4, scst.binom.ppf(0.5, 10, 0.4)),
                (10.0, [0.5, 0.6], 0.4, scst.binom.ppf([0.5, 0.6], 10, 0.4))  # 1d array for x
            ]
        }

    def test_custom_class_1(self):
        d = BinomialEx(total_count=10, probs=0.4)
        actual = d.icdf(torch.tensor(0.3822806015999999)).numpy()
        expected = torch.tensor(3.0).numpy()
        self.assertAlmostEqual(actual, expected, places=4, 
                                msg=f"Expected {expected}, got {actual}")

    def test_custom_class_2(self):
        d = BinomialEx(total_count=10, probs=0.4)
        actual = d.icdf(torch.tensor(0.04635740159999998)).numpy()
    ...
    def test_custom_class_3(self):
        d = BinomialEx(total_count=10, probs=0.4)
        actual = d.icdf(torch.tensor([0.04635740159999998, 0.3822806015999999])).numpy()
        expected = torch.tensor([1.0, 3.0]).numpy()
