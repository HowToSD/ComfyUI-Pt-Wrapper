...
class PtMax:
    ...
    def f(self, tens: torch.Tensor, dim: str, keepdim: bool) -> tuple:
        """
        Computes the maximum values along the specified dimension(s).

        Args:
            tens (torch.Tensor): The input tensor.
            dim (str): A string representation of the dimension(s) along which to compute the maximum,
                       e.g., "0", "[0]", or "(1, 2)".
            keepdim (bool): Whether to retain reduced dimensions.

        Returns:
            tuple: A tuple containing the tensor with maximum values.
        """
        dim = str_to_dim(dim)
        ...
        # Compute the max along the specified dimension(s)
        tens = torch.amax(tens, dim=dim, keepdim=keepdim)
