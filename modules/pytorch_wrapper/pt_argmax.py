...
class PtArgmax:
    ...
    def f(self, tens: torch.Tensor, dim: str, keepdim: bool) -> tuple:
        """
        Computes the indices of the maximum values along the specified dimension(s).

        Args:
            tens (torch.Tensor): The input tensor.
            dim (str): A string representation of the dimension(s) along which to compute argmax,
                       e.g., "0" or "".
            keepdim (bool): Whether to retain reduced dimensions.

        Returns:
            tuple: A tuple containing the tensor with indices of maximum values.
        """
        dim = str_to_dim(dim)
        ...
        # Compute the argmax along the specified dimension(s)
        tens = torch.argmax(tens, dim=dim, keepdim=keepdim)
